...
function main() {
  ...
  console.log(`📂 Loading profile data from: ${PROFILE_DIR}`);
  let profileData;
  try {
    profileData = loadAllProfileData();
    console.log(`✅ Successfully loaded ${Object.keys(profileData).length} profile files`);
  } catch (error) {
  ...
  console.log('🔧 Generating use-ai-settings.js...');
  try {
    const aiSettingsContent = generateAiSettingsModule(profileData);
    writeFile(OUTPUT_FILES.aiSettings, aiSettingsContent);
    console.log(`✅ Generated: ${path.relative(WORKSPACE_ROOT, OUTPUT_FILES.aiSettings)}`);
  } catch (error) {
    console.error(`❌ Error: ${error.message}`);
    return 1;
  }
